...
def sample_spherical_points_with_zero_centroid(n, r=6.37, max_iter=1000, tol=1e-6):
    ...
    vecs = r * vecs / np.linalg.norm(vecs, axis=1, keepdims=True)
    for _ in range(max_iter):
        vecs = vecs - np.mean(vecs, axis=0) 
        vecs = r * vecs / np.linalg.norm(vecs, axis=1, keepdims=True)
        if np.linalg.norm(np.mean(vecs, axis=0)) < tol:
            break
    return vecs
...
class MCF(pl.LightningModule, _DPFMetrics):
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            print(f"Warning: Could not create visualization directory '{self.viz_dir}' due to PermissionError. Evaluation results (COV/MAT) will still be calculated.")
            pass 
        ...
        self.ssd_noise_ratio = 0.01  # noise_ratio
        self.ssd_shell_radius = 6.37 # QM9
        ...
        # Gamma Schedule (SSD)
        self.register_buffer("gamma_schedule", torch.linspace(0.0001, 1.0, self.num_timesteps))
    ...
    def training_step(self, batch, batch_idx, noise=None):
        ...
        x = batch[:, :, : -self.input_signal_num_channels] #feature
        pos = batch_data[:, :, -self.input_signal_num_channels :] #pos shape:(B,N,3)
        ...
        for i in range(batch_size):
            ...
            shell_np = sample_spherical_points_with_zero_centroid(n=n_atoms, r=self.ssd_shell_radius)
            shell_tensor = torch.tensor(shell_np, dtype=torch.float32, device=device)
            if n_atoms < n_nodes:
                padding = torch.zeros((n_nodes - n_atoms, 3), device=device)
                shell_tensor = torch.cat([shell_tensor, padding], dim=0)
            shell_pos_list.append(shell_tensor)
        
        random_shell_pos = torch.stack(shell_pos_list) # (B, N, 3)
        ...
        # (B, 1, 1)
        gamma_t_pos = gamma_t.view(-1, 1, 1)


        # pos_perturbed and target 
        # Add Noise
        if self.ssd_add_noise:

            pos_noise = torch.randn_like(pos)
            target_term = ((1 - self.ssd_noise_ratio) * (pos - random_shell_pos) + self.ssd_noise_ratio * pos_noise)
            pos_perturbed = pos - gamma_t_pos * target_term
            
        # w/o Noise
        else:
            pos = center_pos(pos, batch) 
            target_term = (pos - random_shell_pos)
            pos_perturbed = pos - gamma_t_pos * target_term
        ...
        context_pos_perturbed = pos_perturbed[:, context_indices]
        ...
        query_target = target_term[:, query_indices]
