...
class CFConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_length, edge_attr, return_attention=False):
        if self.smooth:
        ...
        if return_attention:
            # 將 W 的多通道權重簡化為一維分數
            attn_weights = W.norm(dim=-1)  # 取 L2 範數，形狀: (num_edges,)
            attn_weights = torch.softmax(attn_weights, dim=0)  # 歸一化
...
class InteractionBlock(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_length, edge_attr, return_attention=False):
        if return_attention:
            x, attn_weights = self.conv(x, edge_index, edge_length, edge_attr, return_attention=True)
            x = self.act(x)
            x = self.lin(x)
            return x, attn_weights
        else:
...
class SchNetEncoder(Module):
    ...
    def forward(self, z, edge_index, edge_length, edge_attr, embed_node=True, return_attention=False):
        if embed_node:
        ...
        for interaction in self.interactions:
            if return_attention:
                h, attn_weights = interaction(h, edge_index, edge_length, edge_attr, return_attention=True)
        ...
        if return_attention:
            # 平均所有交互層的注意力分數
            attn_weights = torch.stack(attn_weights_list, dim=0).mean(dim=0)  # 形狀: (num_edges,)
