...
# == shell function ==
def normalize_to_radius(vecs, r):
    norms = np.linalg.norm(vecs, axis=1, keepdims=True)
    return r * vecs / norms

def correct_centroid(vecs):
    centroid = np.mean(vecs, axis=0)
    return vecs - centroid

def sample_spherical_points_with_zero_centroid(n, r=6.37, max_iter=1000, tol=1e-6):
    vecs = np.random.randn(n, 3)
    vecs = normalize_to_radius(vecs, r)

    for _ in range(max_iter):
        vecs = correct_centroid(vecs)

        vecs = normalize_to_radius(vecs, r)

        centroid = np.mean(vecs, axis=0)
        if np.linalg.norm(centroid) < tol:
            break
...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.model_type == 'diffusion':
            ...
            # gamma schedule for subtractive diffusion
            T = config.num_diffusion_timesteps
            schedule_type = config.gamma_schedule_type  # "linear", "exp_inc", "exp_dec"
            t = torch.arange(1, T + 1, dtype=torch.float32)

            if schedule_type == "linear":
                gamma = torch.linspace(0.0001, 1.0, self.num_timesteps, dtype=torch.float32)
                # gamma = torch.arange(1, T + 1, dtype=torch.float32) / (T + 1)
            elif schedule_type == "exp_inc":
                beta = 5.0
                gamma_orig = (torch.exp(beta * t / T) - 1) / (torch.exp(torch.tensor(beta)) - 1)
                min_val = 0.1
                max_val = 1.0
                gamma = gamma_orig * (max_val - min_val) + min_val
            elif schedule_type == "exp_dec":
                beta = 5.0
                gamma = 1.0 - (torch.exp(-beta * t / T) - torch.exp(-torch.tensor(beta))) / (1 - torch.exp(-torch.tensor(beta)))
            elif schedule_type == "cosine":
                steps = torch.arange(T, dtype=torch.float32)
                x_cosine = torch.cos(steps / (T - 1) * math.pi)
                gamma = 0.5 * (1 - x_cosine)
                gamma = gamma * (1 - 0.0001) + 0.0001
            elif schedule_type == 'sqrt':
                steps = torch.arange(T, dtype=torch.float32)
                x_norm = steps / (T - 1) # 1. Normalize steps to [0, 1]
                x_sqrt = torch.sqrt(x_norm)  # 2. Apply square root to get concave growth # ∈ [0, 1]
                gamma = x_sqrt * (1 - 0.0001) + 0.0001 # 3. Rescale to [0.0001, 1]
            elif schedule_type == "sigmoid":
                # Sigmoid centered at t=0.5
                sharpness = 10
                s = 1 / (1 + np.exp(-sharpness * (t / T - 0.5)))
                # Scale to desired range
                max_val = 1
                min_val = 0.1
                gamma = min_val + (max_val - min_val) * s
            else:
                raise ValueError(f"Unknown gamma_schedule_type: {schedule_type}")

            self.gamma_schedule = nn.Parameter(gamma, requires_grad=False)
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step, 
        ...
        # print('edge index / type / length shape', edge_index.shape, edge_type.shape, edge_length.shape)
    ...
    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
        ...
        time_step = torch.randint(0, self.num_timesteps, size=(num_graphs,), device=pos.device)
        
        # gamma = (1.0 - a_pos).sqrt() / a_pos.sqrt()
        gamma_t = self.gamma_schedule.index_select(0, time_step)
        gamma_t_pos = gamma_t.index_select(0, node2graph).unsqueeze(-1)

        # sample point on shell
        random_shell_pos = list()
        for num in num_nodes_per_graph:
            point_np = sample_spherical_points_with_zero_centroid(n=num)
            point_tensor = torch.tensor(point_np, dtype=torch.float32)
            random_shell_pos.append(point_tensor)
        random_shell_pos = torch.cat(random_shell_pos, dim=0)
        random_shell_pos = random_shell_pos.to(pos.device)
        assert random_shell_pos.shape == pos.shape, f"Shape mismatch: {pos.shape} vs {random_shell_pos.shape} (pos vs random_shell_pos)"

        pos = center_pos(pos, batch) 
        # pos_perturbed = pos - gamma_t_pos * (pos - random_shell_pos)
        ...
        pos_noise = torch.zeros(size=pos.size(), device=pos.device)
        pos_noise.normal_()
        pos_perturbed = pos - gamma_t_pos * ((1 - self.noise_ratio) * (pos - random_shell_pos) + self.noise_ratio * pos_noise)
        ...
        # edge2graph = node2graph.index_select(0, edge_index[0]) # edge 20794
        # # Compute sigmas_edge
        # a_edge = a.index_select(0, edge2graph).unsqueeze(-1)  # (E, 1)
        edge2graph = node2graph.index_select(0, edge_index[0])
        gamma_t_edge = gamma_t.index_select(0, edge2graph).unsqueeze(-1)
        ...
        d_target = (d_gt - d_perturbed) / gamma_t_edge  # (E_global, 1), denoising direction
    ...
    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
                                 n_steps=500, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None, min_sigma=0, is_sidechain=None,
                                 global_start_sigma=float('inf'), w_global=0.2, w_reg=1.0, **kwargs):
        ...
        sigmas = self.gamma_schedule.to(pos_init.device)
        pos_traj = []
        ...
        with torch.no_grad():
            ...
            random_shell_pos = list()
            point_np = sample_spherical_points_with_zero_centroid(n=pos_init.size(0))
            point_tensor = torch.tensor(point_np, dtype=torch.float32)
            random_shell_pos.append(point_tensor)
            random_shell_pos = torch.cat(random_shell_pos, dim=0)
            random_shell_pos = random_shell_pos.to(pos_init.device)
            assert random_shell_pos.shape == pos_init.shape, f"Shape mismatch: {pos_init.shape} vs {random_shell_pos.shape} (pos vs random_shell_pos)"
            pos_init = random_shell_pos
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                if sigmas[i] < global_start_sigma:
                    ...
                    node_eq_global = 0
                    node_eq_global = torch.tensor([[0, 0, 0]]).cuda()
                # Sum
